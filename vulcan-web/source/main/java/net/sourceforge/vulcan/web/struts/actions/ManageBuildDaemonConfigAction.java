...
public final class ManageBuildDaemonConfigAction extends SchedulerConfigBaseAction {
	@Override
	protected SchedulerConfigDto getConfig(String name) {
		return stateManager.getBuildDaemonConfig(name);
	}

	@Override
	protected void addScheduler(SchedulerConfigDto config) throws DuplicateNameException, StoreException {
		stateManager.addBuildDaemonConfig(config);
	}
	...
	protected void updateSchedulerConfig(String originalName,
		...
		stateManager.updateBuildDaemonConfig(originalName, config);
	}
	
	@Override
	protected void deleteSchedulerConfig(String name) throws StoreException {
		stateManager.deleteBuildDaemonConfig(name);
	}
}
