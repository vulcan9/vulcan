...
class JabberBuildStatusListener implements BuildStatusListener {
	...
	public JabberBuildStatusListener(ProjectStatusDto status) {
		this.status = status;
	}
	
	public void onBuildPhaseChanged(BuildPhase phase) {
		LOG.info("Project " + status.getName() + " entering phase " + phase.name());
	}
	
	public void onErrorLogged(BuildMessageDto error) {
		LOG.info("Project " + status.getName() + " logged error " + error.getMessage());
	}
	
	public void onWarningLogged(BuildMessageDto warning) {
		LOG.info("Project " + status.getName() + " logged warning " + warning.getMessage());
	}
}